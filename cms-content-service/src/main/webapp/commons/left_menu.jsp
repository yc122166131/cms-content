<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
<div id="sidebar-left" class="span2">
		<div class="nav-collapse sidebar-nav">
			<ul class="nav nav-tabs nav-stacked main-menu">
				<li><a href="index.jsp"><i class="icon-align-justify"></i><span class="hidden-tablet"> Tables</span></a></li>
				<li><a href="gallery.jsp"><i class="icon-picture"></i><span class="hidden-tablet"> Gallery</span></a></li>
				<li><a href="form.jsp"><i class="icon-picture"></i><span class="hidden-tablet">form</span></a></li>
				<li><a href="ui.jsp"><i class="icon-picture"></i><span class="hidden-tablet"> UI</span></a></li>
			</ul>
		</div>
</div>
